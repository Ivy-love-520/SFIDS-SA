...
for i = 1:K  
    ...
    if area_ratio > 0.5  
        ...
        %AM_imf_data(AM_imf_data > neighbor_avg) = 0;
        %AM_imf_data_mix = AM_imf_data*0.05;
        %平滑处理
        %AM_imf_data = smooth(AM_imf_data);
        %AM_imf_data = sgolayfilt(AM_imf_data_mix,3,11);
        windowSize = 5;  
        AM_imf_data = movmean(AM_imf_data, windowSize);
        %AM_imf_data = movmedian(AM_imf_data_mix,7);
...
for i = 1:K  
    ...
    plot((f(1:id)), AM_imf_data(1:id));% 绘制频率域图  
    xlabel('Frequency (Hz)');  
    ylabel('Amplitude');  
...
for i = 1:K
    eval(['AM_imf_data = abs(fft(', 'imf', num2str(i), '));']);  
    AM_imf_data = eval(['AM_imf', num2str(i)]);  
    % 在此处添加smooth处理，这里假设使用默认的平滑参数，你可以根据实际需求调整参数
    AM_imf_data = smooth(AM_imf_data); 
     % 绘图
    subplot(K, 1, i);
    plot((f(1:id)), AM_imf_data(1:id)); % 绘制频率域图，这里展示部分频率范围，和你原代码保持一致，你也可按需调整
    xlabel('Frequency (Hz)');
    ylabel('Amplitude');
    title(['Smooth IMF Frequency Domain - IMF', num2str(i)]); % 修改标题表明是平滑后的频率域
    eval(['AM_imf', num2str(i), ' = AM_imf_data;']);  % 更新IMF数据  
    % 更新原始IMF（如果需要保持一致）等代码保持不变，此处省略部分重复代码展示
end
% 新建一个图形窗口用于展示逆变换后的结果（这里可根据实际需求选择合适的展示方式和窗口编号等）
figure(6);
% 循环遍历每个IMF分量进行傅里叶逆变换及相关操作
for i = 1:K
    % 获取当前的AM_imf数据（假设之前的平滑处理已经将平滑后的数据存到了对应的AM_imf变量中）
    current_AM_imf = eval(['AM_imf', num2str(i)]);
    % 进行傅里叶逆变
    Z=fft(s2);
    Y=(current_AM_imf)'.*exp(1i*angle(Z));
    Reconstruction_imf=real(ifft(Y)); %% 求取逆傅里叶变换
    ...
    subplot(K, 1, i);
    plot(t, Reconstruction_imf); % 绘制时域波形，取实部展示，ifft结果可能有虚部，按需决定是否只展示实部
    xlabel('Time (s)');
    ylabel('Amplitude');
...
% % 绘制GST处理后的每个inverse_AM_imf_data的时频谱图
% for i = 1:K
%     % 创建新的图像窗口
%     figure(13+i);
%     temp = abs(TFRgst_inverse_AM_imf(:, 1:id)');
%     tfr = temp./ max(max(temp));
%     imagesc(t, f(1:id), tfr);
%     xlabel('Time(s)');
%     ylabel('Frequency(Hz)');
%     title(['GST of Inverse Fourier Transform of AM_imf', num2str(i)]);
%     set(gca, 'ydir', 'normal');
%     colormap jet
% end
...
% %信号重构方法2（使用时间域重构信号）
% Reconstruction_imf_sum = 0; % 根据你的变量类型初始化合适的零值，这里以数值为例，若为矩阵等需要改成zeros初始化相应大小的零矩阵等
% for i = 1:K
%     Reconstruction_imf_sum = Reconstruction_imf_sum + eval(sprintf('Reconstruction_imf%d', i));
% end
...
%dlmwrite('reconstructed_signal_y.txt', y); 
