...
imf_VMD=imf2+imf3+imf4+imf5+imf6+imf7;
...
figure;
hold on;
colors = lines(K); % 使用与之前一致的颜色映射

% % 统一频率轴范围（根据前文设置的freq=250Hz）
f_plot = f(1:id); % 截取到最大显示频率

for i = 1:K
    % 动态获取处理后的AM_imf数据
    current_AM = eval(['AM_imf', num2str(i), '(1:id)']); 
    
    % 沿Y轴堆叠IMF
    y = i * ones(size(f_plot));
    
    % 三维绘图
    plot3(f_plot, y, current_AM,...
         'Color', colors(i,:),...
         'LineWidth', 1.5);
end
...
ylabel('IMF Number');
zlabel('Amplitude');
title('3D Frequency Spectrum of Processed IMFs');
view(35, 30); % 优化视角
grid on;
set(gca, 'FontSize', 10); % 统一字体大小
hold off;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
imf_sum=0;
for i = 1:K
...
figure;
subplot(2,1,1);
plot(t,xz,'b');xlabel('Time(s)');ylabel('Amplitude');
...
title('原始含噪信号');
%% Perform K-SVD
NoiseSigma = NoiseEstimate(Sig1);
Params.n = 128;
Params.m = Params.n * 2;
Params.E = 20 * NoiseSigma * getConstant(Params.n);
tic
[y_KSVD1] = CleanKSVD(Sig1, Params);
toc
figure;
plot(t,y_KSVD1,'b');
xlabel('Time(s)');ylabel('Amplitude')
...
%     id = find(f <= max_freq, 1, 'last');
    ...
    figure; % 创建新的图像窗口
    % 计算 IMF1 的频率域数据
    AM_imf1 = abs(fft(imfevv));
    AM_imf_data1 = abs(fft(imf1));
    %绘制 AM_imf1 频谱 (红色)
    plot(f(1:id), AM_imf1(1:id,1), 'r', 'LineWidth', 1.5); hold on;
    % 绘制 AM_imf_data1 频谱 (蓝色)
    plot(f(1:id), AM_imf_data1(1:id), 'b', 'LineWidth', 1.5);
    % 轴标签和图例
    xlabel('Frequency (Hz)');
    xlim([0 150]);
    ylabel('Amplitude');
    title('IMF1 Frequency Domain');
    legend('AM\_imf1', 'AM\_imf\_data1'); % 添加图例区分两条曲线
    hold off; % 结束绘制
    AMimf_sum = abs(fft(imf_sum));
    %单频压制前后的时域，频率域及时频谱图
    figure;
    subplot(3,1,1);
    plot(t,imf_sum,'b');xlim([0 3]);    
    set(gca, 'FontName', 'Times New Roman', 'FontSize', 18);
    xlabel('Time(s)', 'FontName', 'Times New Roman', 'FontSize', 18);
    ylabel('Amplitude', 'FontName', 'Times New Roman', 'FontSize', 18);
    subplot(3,1,2);
    plot((f(1:id)),AMimf_sum(1:id),'b');
    set(gca, 'FontName', 'Times New Roman', 'FontSize', 18);
    xlabel('Time(s)', 'FontName', 'Times New Roman', 'FontSize', 18);
    ylabel('Amplitude', 'FontName', 'Times New Roman', 'FontSize', 18);
    TFRgst_imf_sum = TFT(imf_sum,[],dt,'GST',0.2,20);
    subplot(3,1,3);
    temp = abs(TFRgst_imf_sum(:,1:id)');
    tfr = temp ./ max(max(temp));
    imagesc(t,f(1:id),tfr);
    %xlabel('Time(s)');ylabel('Frequency(Hz)');
    xlim([0 3]);
    ...
    xlim([0 3]);
    subplot(9,3,4);
    plot((f(1:id)),AM_VMD(1:id),'b');
    xlabel('Frequency(Hz)');
    ylabel('Amplitude');
    subplot(9, 3, 7);
    temp = abs(TFRgst_imf_VMD(:,1:id)');
    tfr = temp ./ max(max(temp));
    imagesc(t,f(1:id),tfr);
    %xlabel('Time(s)');ylabel('Frequency(Hz)');
    xlim([0 3]);
...
% 获取原始的jet颜色映射
cmap = jet; 
% 调整颜色亮度（这里通过线性变换调整，系数和偏移可根据实际微调）
brightness_factor = 1.1; 
offset = 0.05;
cmap = min(1, cmap * brightness_factor + offset); 
colormap(cmap);
